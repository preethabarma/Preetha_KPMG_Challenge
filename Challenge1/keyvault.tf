...
resource "azurerm_key_vault" "demo-keyvault" {
  name                        = "demotestkeyvault01"
...
resource "azurerm_key_vault_access_policy" "kv_access_policy_01" {
  ...
  tenant_id          = data.azurerm_client_config.current.tenant_id
  object_id          = "23959414-bf1a-4e03-ab2e-d73ac3bd6904"
  key_permissions    = ["Get", "List"]
  secret_permissions = ["Get", "Backup", "Delete", "List", "Purge", "Recover", "Restore", "Set"]

  depends_on = [azurerm_key_vault.demo-keyvault]
}
